-- PL/SQL
-- 내용을 EMPLOYEES 테이블에서 임의의 부서번호를 랜덤으로 생성한 뒤, 해당된 부서번호에 최고연봉을 출력한 뒤 평가하여라(낮음,높음,중간,최고,없음)
...
BEGIN
    ...
    SELECT SALARY INTO VTOP_SALARY FROM 
    (SELECT SALARY FROM EMPLOYEES WHERE DEPARTMENT_ID = VNO 
...
(SELECT ROWNUM, SALARY, DEPARTMENT_ID, FIRST_NAME FROM EMPLOYEES WHERE DEPARTMENT_ID = 50 ORDER BY SALARY DESC);
