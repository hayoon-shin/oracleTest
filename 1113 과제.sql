-- 문제 1) 사원 번호와 사원명과 부서명과 부서의 위치를 출력하는 뷰(VIEW_LOC)를 작성하라.
...
-- 문제 2) 30번 부서 소속 사원의 이름과 입사일과 부서명을 출력하는 뷰(VIEW_DEPT30)를 작성하라.
...
-- 문제 3) 부서별 최대 급여 정보를 가지는 뷰(VIEW_DEPT_MAXSAL)를 생성하라.
DROP VIEW VIEW_DEPT_MAXSAL;

CREATE VIEW VIEW_DEPT_MAXSAL
AS
SELECT MAX(E.SALARY) AS MAX_SALARY, E.DEPARTMENT_ID
FROM EMPLOYEES E
GROUP BY E.DEPARTMENT_ID; 

SELECT * FROM VIEW_DEPT_MAXSAL;

-- 문제 4) 급여를 많이 받는 순서대로 3명만 출력하는 뷰(VIEW_SAL_TOP3)와 인라인 뷰로 작성하라.
